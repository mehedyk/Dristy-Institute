...
const THEMES = {
  ...
  vivid:    { name:"Electric Violet",  emoji:"💜", bg:"#0D0020", bg2:"#140030", card:"#1C0040", p:"#C026D3", a:"#22D3EE", tx:"#FAF5FF", mu:"#7E22CE", gr:"192,38,211" },
};
...
const scrollTo = id => {
  ...
  if (!el) return;
  window.scrollTo({ top: el.getBoundingClientRect().top + window.scrollY - 98, behavior: "smooth" });
};
...
const L = {
  ...
  en: {
    ...
    stepD: ["Visit or WhatsApp us to get full admission details","Select the course and batch that suits your schedule","Complete your course and build a career with your certificate"],
...
const CSS = `
  ...
  *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
  ...
  body{font-family:'Hind Siliguri','Poppins',system-ui,sans-serif;overflow-x:hidden;line-height:1.6;transition:background .35s,color .35s}
  ...
  @media(max-width:768px){
    ...
    .sm-text-sm{font-size:clamp(28px,7vw,40px)!important}
  }
...
const themeVars = t =>
  `:root{--bg:${t.bg};--bg2:${t.bg2};--card:${t.card};--p:${t.p};--a:${t.a};--tx:${t.tx};--mu:${t.mu};--gr:${t.gr}}body{background:${t.bg};color:${t.tx}}`;
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      trail.forEach(d => {
        ...
        if (d.life <= 0) return;
        ctx.beginPath(); ctx.arc(d.x, d.y, 5 * d.life, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(${rgb},${d.life * 0.75})`; ctx.fill();
      });
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < pts.length; i++) for (let j = i + 1; j < pts.length; j++) {
        ...
        if (d < 95) { ctx.beginPath(); ctx.moveTo(pts[i].x, pts[i].y); ctx.lineTo(pts[j].x, pts[j].y); ctx.strokeStyle = `rgba(${rgb},${.09 * (1 - d / 95)})`; ctx.lineWidth = .5; ctx.stroke(); }
      }
      id = requestAnimationFrame(draw);
    };
    draw();
...
// ─── MOBILE MENU ────────────────────────────────────────────────────
function MobileMenu({ open, onClose, t, c, lang, setLang, themeKey, setThemeKey, cursorOn, setCursorOn }) {
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(320px,88vw)", background: t.bg2, borderLeft: `1px solid rgba(${t.gr},.15)`, zIndex: 1101, transform: open ? "translateX(0)" : "translateX(100%)", transition: "transform .35s cubic-bezier(.16,1,.3,1)", display: "flex", flexDirection: "column", overflowY: "auto" }}>
        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 20px 16px", borderBottom: `1px solid rgba(${t.gr},.1)` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 34, height: 34, borderRadius: 8, background: t.p, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 800, color: t.bg }}>দৃ</div>
          ...
          <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 8, background: `rgba(${t.gr},.1)`, border: `1px solid rgba(${t.gr},.2)`, color: t.mu, fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
        </div>
        {/* Nav links */}
        <div style={{ padding: "12px 12px" }}>
          {c.nav.map((l, i) => (
            <button key={l} onClick={() => { scrollTo(NAV_IDS[i]); onClose(); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "14px 12px", background: "transparent", border: "none", borderRadius: 10, color: t.tx, fontSize: 15, fontWeight: 600, cursor: "pointer", transition: "background .2s", textAlign: "left" }}
              onMouseEnter={e => e.currentTarget.style.background = `rgba(${t.gr},.08)`}
              onMouseLeave={e => e.currentTarget.style.background = "transparent"}
            >
              <span style={{ color: t.p, width: 20 }}>{["📚","✅","📞"][i]}</span>{l}
            </button>
          ))}
        </div>
        {/* Divider */}
        <div style={{ height: 1, background: `rgba(${t.gr},.1)`, margin: "0 12px" }} />
        {/* Toggles */}
        <div style={{ padding: "14px 12px", display: "flex", gap: 8 }}>
          <button onClick={() => setLang(l => l === "bn" ? "en" : "bn")} style={{ flex: 1, padding: "10px", background: `rgba(${t.gr},.1)`, border: `1px solid rgba(${t.gr},.22)`, borderRadius: 9, color: t.p, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{lang === "bn" ? "Switch to EN" : "বাংলায় দেখুন"}</button>
          <button onClick={() => setCursorOn(v => !v)} style={{ padding: "10px 14px", background: cursorOn ? t.p : `rgba(${t.gr},.1)`, border: `1px solid rgba(${t.gr},.22)`, borderRadius: 9, color: cursorOn ? t.bg : t.mu, fontSize: 13, cursor: "pointer" }}>✦ Trail</button>
        </div>
        {/* Theme grid */}
        <div style={{ padding: "0 12px 12px" }}>
          <div style={{ color: t.mu, fontSize: 10, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 10, paddingLeft: 4 }}>THEMES</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
            {Object.entries(THEMES).map(([key, th]) => (
              <button key={key} onClick={() => { setThemeKey(key); onClose(); }} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, padding: "10px 6px", background: themeKey === key ? `rgba(${th.gr},.15)` : `rgba(${t.gr},.05)`, border: `1px solid ${themeKey === key ? `rgba(${th.gr},.4)` : `rgba(${t.gr},.1)`}`, borderRadius: 10, cursor: "pointer", transition: "all .18s" }}>
                ...
                <span style={{ color: themeKey === key ? th.p : t.mu, fontSize: 9, fontWeight: themeKey === key ? 700 : 400, lineHeight: 1.2, textAlign: "center" }}>{th.emoji}<br />{th.name.split(" ")[0]}</span>
              </button>
            ))}
          </div>
        </div>
        {/* CTA buttons */}
        <div style={{ padding: "0 12px 28px", marginTop: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
          <button onClick={() => { window.open("tel:01643928687"); onClose(); }} style={{ width: "100%", padding: "14px", background: t.p, color: t.bg, border: "none", borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: "pointer", boxShadow: `0 0 18px rgba(${t.gr},.4)` }}>{c.enroll}</button>
          <a href="tel:01643928687" style={{ width: "100%", padding: "13px", background: `rgba(${t.gr},.1)`, color: t.p, border: `1px solid rgba(${t.gr},.25)`, borderRadius: 10, fontWeight: 600, fontSize: 14, cursor: "pointer", textDecoration: "none", textAlign: "center", display: "block" }}>📞 01643-928687</a>
        </div>
      </div>
    </>
  );
...
// ─── NAVBAR ─────────────────────────────────────────────────────────
function Navbar({ t, c, lang, setLang, themeKey, setThemeKey, cursorOn, setCursorOn, menuOpen, setMenuOpen }) {
  const [sc, setSc] = useState(false);
  const [tOpen, setTOpen] = useState(false);
  const tRef = useRef(null);
  useClickOutside(tRef, useCallback(() => setTOpen(false), []));
  useEffect(() => {
    ...
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 998 }}>
      ...
      <nav style={{ height: 62, padding: "0 5%", display: "flex", alignItems: "center", justifyContent: "space-between", transition: "all .35s", background: sc ? `${t.bg}F0` : "transparent", backdropFilter: sc ? "blur(18px)" : "none", borderBottom: sc ? `1px solid rgba(${t.gr},.12)` : "1px solid transparent" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {/* Language */}
          <button onClick={() => setLang(l => l === "bn" ? "en" : "bn")} style={{ background: `rgba(${t.gr},.09)`, border: `1px solid rgba(${t.gr},.22)`, color: t.p, padding: "6px 11px", borderRadius: 7, fontSize: 11, fontWeight: 700, cursor: "pointer", transition: "all .2s", whiteSpace: "nowrap" }}
          ...
          {/* Theme picker */}
          <div ref={tRef} style={{ position: "relative" }}>
            <button onClick={() => setTOpen(o => !o)} style={{ width: 30, height: 30, borderRadius: 7, background: `rgba(${t.gr},.09)`, border: `1px solid rgba(${t.gr},.22)`, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 2, transition: "all .2s" }}
              title="Change Theme"
              onMouseEnter={e => e.currentTarget.style.background = `rgba(${t.gr},.18)`} onMouseLeave={e => e.currentTarget.style.background = `rgba(${t.gr},.09)`}
            >
            ...
            {tOpen && (
              <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: `${t.bg2}F5`, backdropFilter: "blur(20px)", border: `1px solid rgba(${t.gr},.18)`, borderRadius: 13, padding: "12px 10px", width: 210, boxShadow: "0 8px 40px rgba(0,0,0,.45)", animation: "fadeIn .2s ease", zIndex: 100 }}>
                <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: ".1em", color: t.mu, textTransform: "uppercase", marginBottom: 8, paddingLeft: 4 }}>THEMES</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>
                  {Object.entries(THEMES).map(([key, th]) => (
                    <button key={key} onClick={() => { setThemeKey(key); setTOpen(false); }} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: "8px 4px", background: themeKey === key ? `rgba(${th.gr},.18)` : "transparent", border: `1px solid ${themeKey === key ? `rgba(${th.gr},.4)` : "transparent"}`, borderRadius: 8, cursor: "pointer", transition: "all .16s" }}
                      onMouseEnter={e => { if (themeKey !== key) e.currentTarget.style.background = `rgba(${t.gr},.08)`; }}
                      onMouseLeave={e => { if (themeKey !== key) e.currentTarget.style.background = "transparent"; }}
                    >
                      <div style={{ display: "flex", gap: 2 }}>
                        <div style={{ width: 9, height: 9, borderRadius: "50%", background: th.p }} />
                        <div style={{ width: 9, height: 9, borderRadius: "50%", background: th.a }} />
                      </div>
                      <span style={{ color: themeKey === key ? th.p : t.mu, fontSize: 8.5, fontWeight: themeKey === key ? 700 : 400, textAlign: "center", lineHeight: 1.3 }}>{th.emoji} {th.name.split(" ").slice(0, 2).join(" ")}</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <button onClick={() => window.open("tel:01643928687")} className="hide-sm" style={{ background: t.p, color: t.bg, border: "none", padding: "8px 18px", borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: "pointer", boxShadow: `0 0 13px rgba(${t.gr},.38)`, transition: "all .22s", whiteSpace: "nowrap" }}
            onMouseEnter={e => { e.target.style.transform = "translateY(-1px)"; e.target.style.boxShadow = `0 0 22px rgba(${t.gr},.55)`; }}
            onMouseLeave={e => { e.target.style.transform = "none"; e.target.style.boxShadow = `0 0 13px rgba(${t.gr},.38)`; }}
          >{c.enroll}</button>
          {/* Hamburger (mobile) */}
          <button onClick={() => setMenuOpen(true)} style={{ display: "none", width: 36, height: 36, borderRadius: 8, background: `rgba(${t.gr},.09)`, border: `1px solid rgba(${t.gr},.2)`, cursor: "pointer", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4 }} className="show-sm">
            {[0,1,2].map(i => <div key={i} style={{ width: 18, height: 2, borderRadius: 1, background: t.p }} />)}
          </button>
        </div>
      </nav>
    </div>
      ...
      <div style={{ maxWidth: 1140, margin: "0 auto", width: "100%", position: "relative", zIndex: 2 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 52, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 260 }} className="sm-full sm-center">
            ...
            <h1 className="sm-text-sm" style={{ ...a("0.1s"), fontWeight: 900, fontSize: "clamp(34px,5.2vw,62px)", lineHeight: 1.08, marginBottom: 14 }}>
            ...
            <div style={{ ...a("0.32s"), display: "flex", gap: 12, flexWrap: "wrap" }}>
              <button onClick={() => window.open("tel:01643928687")} style={{ background: t.p, color: t.bg, border: "none", padding: "13px 28px", borderRadius: 9, fontWeight: 700, fontSize: 14, cursor: "pointer", boxShadow: `0 0 20px rgba(${t.gr},.4)`, transition: "all .22s" }}
                onMouseEnter={e => { e.target.style.transform = "translateY(-2px)"; e.target.style.boxShadow = `0 0 32px rgba(${t.gr},.58)`; }}
                onMouseLeave={e => { e.target.style.transform = "none"; e.target.style.boxShadow = `0 0 20px rgba(${t.gr},.4)`; }}
              >{c.btn1}</button>
              <button onClick={() => window.open("tel:01643928687")} style={{ background: "transparent", color: t.tx, border: `1.5px solid rgba(${t.gr},.28)`, padding: "12px 24px", borderRadius: 9, fontWeight: 600, fontSize: 14, cursor: "pointer", transition: "all .22s" }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = t.p; e.currentTarget.style.color = t.p; e.currentTarget.style.background = `rgba(${t.gr},.07)`; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = `rgba(${t.gr},.28)`; e.currentTarget.style.color = t.tx; e.currentTarget.style.background = "transparent"; }}
              >{c.btn2}</button>
            </div>
            <div style={{ ...a("0.44s"), display: "flex", alignItems: "center", gap: 12, marginTop: 28, flexWrap: "wrap" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 9, background: `rgba(${t.gr},.07)`, border: `1px solid rgba(${t.gr},.18)`, borderRadius: 10, padding: "9px 16px" }}>
                ...
                <div>
                  <div style={{ color: t.p, fontWeight: 800, fontSize: 15 }}>{c.disc}</div>
                  <div style={{ color: t.mu, fontSize: 11 }}>{c.discSub}</div>
                </div>
              </div>
              {c.trust.map(tr => (
                <div key={tr} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <span style={{ color: t.p, fontSize: 13 }}>✓</span>
                  <span style={{ color: t.mu, fontSize: 12 }}>{tr}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="hide-sm" style={{ opacity: on ? 1 : 0, transform: on ? "none" : "translateX(36px)", transition: "all .9s cubic-bezier(.16,1,.3,1) .42s", width: 282, flexShrink: 0 }}>
            <div style={{ background: `rgba(255,255,255,.04)`, backdropFilter: "blur(20px)", border: `1px solid rgba(${t.gr},.16)`, borderRadius: 17, padding: "22px 20px", boxShadow: `0 0 44px rgba(${t.gr},.07),inset 0 1px 0 rgba(255,255,255,.05)` }}>
              ...
              <button onClick={() => window.open("tel:01643928687")} style={{ width: "100%", padding: "10px", borderRadius: 8, background: t.p, color: t.bg, border: "none", fontWeight: 700, fontSize: 12, cursor: "pointer", marginTop: 4, transition: "all .22s", boxShadow: `0 0 12px rgba(${t.gr},.32)` }}
                onMouseEnter={e => { e.target.style.transform = "translateY(-1px)"; e.target.style.boxShadow = `0 0 22px rgba(${t.gr},.5)`; }}
                onMouseLeave={e => { e.target.style.transform = "none"; e.target.style.boxShadow = `0 0 12px rgba(${t.gr},.32)`; }}
              >{c.enroll} →</button>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(270px,1fr))", gap: 22 }} className="sm-g1">
          {c.courses.map((cr, i) => (
            <div key={i} className={`card sr d${i+1}`} style={{ background: t.card, border: `1px solid rgba(${t.gr},.${i===1?"22":"1"})`, borderRadius: 16, overflow: "hidden", boxShadow: i===1 ? `0 0 28px rgba(${t.gr},.1)` : "none", position: "relative" }}
              onMouseEnter={e => e.currentTarget.style.boxShadow = `0 14px 44px rgba(${t.gr},.16)`}
              onMouseLeave={e => e.currentTarget.style.boxShadow = i===1 ? `0 0 28px rgba(${t.gr},.1)` : "none"}
            >
              ...
              <div style={{ padding: "0 20px 20px" }}>
                ...
                <button onClick={() => window.open("tel:01643928687")} style={{ width: "100%", padding: "10px", borderRadius: 8, background: i===1 ? t.p : `rgba(${t.gr},.09)`, color: i===1 ? t.bg : t.p, border: `1px solid rgba(${t.gr},.2)`, fontWeight: 700, fontSize: 12, cursor: "pointer", transition: "all .22s" }}
                  onMouseEnter={e => { e.target.style.background = t.p; e.target.style.color = t.bg; e.target.style.boxShadow = `0 0 16px rgba(${t.gr},.38)`; }}
                  onMouseLeave={e => { e.target.style.background = i===1 ? t.p : `rgba(${t.gr},.09)`; e.target.style.color = i===1 ? t.bg : t.p; e.target.style.boxShadow = "none"; }}
                >{c.cBtn}</button>
              </div>
            </div>
          ))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 18 }} className="sm-g1">
          {c.features.map((f, i) => (
            <div key={i} className={`card sr d${i+1}`} style={{ background: t.card, border: `1px solid rgba(${t.gr},.1)`, borderRadius: 13, padding: "18px", display: "flex", gap: 14, alignItems: "flex-start", transition: "border-color .25s,box-shadow .25s" }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = `rgba(${t.gr},.28)`; e.currentTarget.style.boxShadow = `0 8px 30px rgba(${t.gr},.1)`; }}
            ...
            >
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize: 14, color: t.tx, marginBottom: 4 }}>{f.t}</div>
                <div style={{ color: t.mu, fontSize: 12, lineHeight: 1.65 }}>{f.d}</div>
              </div>
            </div>
          ))}
        </div>
          ...
          {c.steps.map((s, i) => (
            <div key={i} className={`sr d${i+1}`} style={{ textAlign: "center", position: "relative", zIndex: 1 }}>
              <div style={{ width: 66, height: 66, borderRadius: "50%", background: t.card, border: `2px solid rgba(${t.gr},.18)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, margin: "0 auto 16px", transition: "border-color .25s,box-shadow .25s", position: "relative" }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = t.p; e.currentTarget.style.boxShadow = `0 0 20px rgba(${t.gr},.22)`; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = `rgba(${t.gr},.18)`; e.currentTarget.style.boxShadow = "none"; }}
              >
                {["📞","📋","🎓"][i]}
      ...
      <div style={{ display: "flex", gap: 18, width: "max-content", animation: "marquee 32s linear infinite" }}
        ...
        {doubled.map((item, i) => (
          <div key={i} className="card" style={{ width: 300, flexShrink: 0, background: t.card, border: `1px solid rgba(${t.gr},.1)`, borderRadius: 14, padding: "20px", transition: "border-color .25s" }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = `rgba(${t.gr},.28)`; e.currentTarget.style.boxShadow = `0 10px 30px rgba(${t.gr},.1)`; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = `rgba(${t.gr},.1)`; e.currentTarget.style.boxShadow = "none"; }}
          >
        ...
        <div style={{ position: "relative", zIndex: 2, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 32 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <button onClick={() => window.open("tel:01643928687")} style={{ background: t.p, color: t.bg, border: "none", padding: "15px 36px", borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: "pointer", transition: "all .25s", boxShadow: `0 0 20px rgba(${t.gr},.38)` }}
              onMouseEnter={e => { e.target.style.transform = "translateY(-2px)"; e.target.style.boxShadow = `0 0 38px rgba(${t.gr},.55)`; }}
              onMouseLeave={e => { e.target.style.transform = "none"; e.target.style.boxShadow = `0 0 20px rgba(${t.gr},.38)`; }}
            >{c.ctaBtn}</button>
  ...
  return (
    <footer style={{ background: t.bg2, borderTop: `1px solid rgba(${t.gr},.08)` }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "52px 5% 24px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 40, marginBottom: 36 }} className="sm-g1 sm-col">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize: 13, color: t.tx }}>Dristy Computer Training Center</div>
                <div style={{ fontSize: 10, color: t.mu }}>Gopalpur, Tangail, Bangladesh</div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {[["📘","Facebook","https://www.facebook.com/share/1E2jAYRKhz/"],["📞","01643-928687","tel:01643928687"]].map(([ic,l,hr]) => (
                <a key={l} href={hr} style={{ background: `rgba(${t.gr},.07)`, border: `1px solid rgba(${t.gr},.14)`, borderRadius: 7, padding: "6px 11px", color: t.mu, fontSize: 11, textDecoration: "none", transition: "all .2s", display: "flex", alignItems: "center", gap: 5 }}
                  onMouseEnter={e => { e.currentTarget.style.background = `rgba(${t.gr},.14)`; e.currentTarget.style.color = t.p; e.currentTarget.style.borderColor = `rgba(${t.gr},.28)`; }}
                  onMouseLeave={e => { e.currentTarget.style.background = `rgba(${t.gr},.07)`; e.currentTarget.style.color = t.mu; e.currentTarget.style.borderColor = `rgba(${t.gr},.14)`; }}
                >{ic} {l}</a>
              ))}
            </div>
          </div>
          <div>
            ...
            {c.fLinks.map(l => (
              <div key={l} style={{ color: t.mu, fontSize: 12, marginBottom: 8, cursor: "pointer", transition: "color .2s" }}
        ...
        <div style={{ borderTop: `1px solid rgba(${t.gr},.08)`, paddingTop: 18, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
...
export default function App() {
  ...
  return (
    <>
      ...
      <Navbar t={t} c={c} lang={lang} setLang={setLang} themeKey={themeKey} setThemeKey={setThemeKey} cursorOn={cursorOn} setCursorOn={setCursorOn} menuOpen={menuOpen} setMenuOpen={setMenuOpen} />
      <MobileMenu open={menuOpen} onClose={() => setMenuOpen(false)} t={t} c={c} lang={lang} setLang={setLang} themeKey={themeKey} setThemeKey={setThemeKey} cursorOn={cursorOn} setCursorOn={setCursorOn} />
